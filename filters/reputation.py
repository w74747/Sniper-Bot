...
async def check_deployer_history(deployer_wallet: str) -> DeployerHistoryResult:
    ...
    try:
        ...
        logger.warning(f"فشل فحص سجل المطور تقنياً (سيُتجاهَل بأمان): {e}")
        # إصلاح مهم: هذا الفحص لا يزال TODO (لا يكتشف rug pulls حقيقية بعد —
        # المسار الناجح نفسه يُرجع دائماً known_rugs=0 بلا أي فحص فعلي). كان
        # "الرفض الاحتياطي" هنا منطقياً فقط لو كان الفحص يكتشف شيئاً حقيقياً
        # عند نجاحه — بما أنه لا يفعل ذلك، فإن رفض العملة عند فشله التقني
        # (متكرر جداً بسبب قيود بعض مزودي RPC مثل Chainstack) لا يوفّر أي
        # حماية حقيقية، بل يرفض فرصاً حقيقية بلا داعٍ. لذا: fail-open هنا.
        return DeployerHistoryResult(
            prior_token_launches=0,
            known_prior_rugs=0,
            reason="تعذّر التحقق تقنياً من سجل المطور — تم التخطي بأمان (الفحص نفسه لا يكتشف rugs حقيقية بعد)",
        )
