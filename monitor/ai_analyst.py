...
async def analyze_and_summarize() -> str:
    ...
    system_prompt = (
        ...
        "كمشكلة تقنية محتملة في مصدر السعر. قاعدة إلزامية عند مقارنة "
        "الاستراتيجيات: استخدم **متوسط الربح/الخسارة لكل صفقة** (avg_profit_loss_sol) "
        "دائماً، **وليس** المجموع الخام (total_profit_loss_sol) — لأن الاستراتيجيات "
        "تختلف بشدة في عدد الصفقات (مثلاً 9 مقابل 26)، فمقارنة المجاميع مباشرة "
        "تُعطي انطباعاً مبالغاً فيه جداً عن الفارق الحقيقي (فارق 43% فعلياً في "
        "المتوسط قد يبدو خطأً وكأنه 300%+ عند مقارنة المجاميع الخام). اذكر "
        "المتوسط لكل صفقة صراحة عند إصدار أي حكم بأن استراتيجية \"أفضل\" أو "
        "\"أسوأ\" من أخرى. اكتب تقريراً عربياً مختصراً "
        "(٦-٩ أسطر كحد أقصى) يغطي: 1) هل الوضع صحي عموماً؟ 2) أي استراتيجية "
        "تُظهر أفضل *متوسط* أداء لكل صفقة حتى الآن (إن وُجدت 5+ صفقات لكل واحدة "
        "على الأقل للمقارنة)، وأيّها الأسوأ؟ إن كانت كل الاستراتيجيات عدا واحدة "
        "أو اثنتين تفتقر لعدد كافٍ من الصفقات، اذكر هذا صراحة كنقص بيانات، وليس "
        "حكماً نهائياً. 3) أهم مشكلة تقنية واضحة إن وُجدت (اسم "
