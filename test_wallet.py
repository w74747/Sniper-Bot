...
async def main():
    ...
    try:
        ...
        print()
        print("⚠️  قارن هذا العنوان مع العنوان الظاهر في تطبيق Phantom —")
    ...
    if USE_DEVNET:
        print("⚠️  لا يزال USE_DEVNET=true")
        print("   → لن تُنفَّذ أي معاملة حقيقية حتى تُغيّره لـ false في .env")
    else:
        print("🚨 USE_DEVNET=false")
        print("   → أنت على Mainnet! تأكد من المحفظة قبل التداول!")
