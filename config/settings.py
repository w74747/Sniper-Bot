...
# المفتاح (إن وُجد) يرفع فقط حد المعدل من 10 لـ60 طلب/دقيقة.
RUGCHECK_API_KEY = os.getenv("RUGCHECK_API_KEY", "").strip()
RUGCHECK_MAX_SCORE = 60.0        # رفض إن تجاوزت درجة الخطر هذا الحد (0-100)
RUGCHECK_MAX_INSIDERS = 3         # رفض إن اكتُشف أكثر من هذا العدد من المطلعين المرتبطين

# GMGN OpenAPI — طبقة تحليل إضافية غنية (Smart Money، Rat Trader، Bundler).
# لا حاجة للمفتاح الخاص (GMGN_PRIVATE_KEY) — استعلامات القراءة فقط تحتاج
# مفتاح API البسيط (X-APIKEY)، بدون أي توقيع تشفيري (مؤكَّد من الكود المصدري
# الرسمي لـgmgn-cli، وليس افتراضاً).
GMGN_API_KEY = os.getenv("GMGN_API_KEY", "").strip()
GMGN_MAX_RAT_TRADER_PCT = 30.0   # رفض إن تجاوزت نسبة حجم "المحافظ المُرتزقة" هذا الحد
GMGN_MAX_BUNDLER_PCT = 25.0      # رفض إن تجاوزت نسبة "الشراء المُجمَّع بالبوتات" هذا الحد
GMGN_MAX_RUG_RATIO = 0.6         # رفض إن تجاوزت درجة احتمال rug هذا الحد (0-1)

# حظر الأسماء المُستنسَخة الخطرة (دائم) — اكتشفنا فعلياً 12 عملة مختلفة
# (عناوين مختلفة تماماً) بنفس الاسم "USOH"، بنمط شبه مثالي: ربح ~+51% أو
# خسارة كارثية ~-100% بالتناوب. هذا يوحي بمُصدِر واحد (أو مجموعة) يستغل
# نفس الاسم بشكل مُمنهج. الحظر هنا **دائم** (جدول symbol_blocklist)،
# وليس محدوداً بنافذة زمنية — بمجرد التسجيل، يبقى محظوراً إلى الأبد.
SYMBOL_BLOCKLIST_LOSS_THRESHOLD_PCT = -80.0  # خسارة "كارثية" = أسوأ من هذا
SYMBOL_BLOCKLIST_MAX_OCCURRENCES = 1  # حظر دائم بعد أول خسارة كارثية واحدة فقط بنفس الاسم
...
class ExitStrategySettings:
    ...
    # حماية رأس المال
    max_capital_pct_per_trade: float = 10.0  # رُفِع بناءً على طلب صريح من المستخدم (بعد تنبيه
                                                # واضح: خسارة كارثية واحدة تُكلّف الآن 4 أضعاف ما
                                                # كانت تُكلّفه عند 2.5%، نظراً لأنماط انهيار سيولة
                                                # فورية (73-100%) رُصدت فعلياً في صفقات سابقة)
...
class SustainedTrendSettings:
    ...
    max_price_change_m5_pct: float = 300.0   # إصلاح حرج: كانت هذه الاستراتيجية تفتقد سقف
                                              # الزخم المتطرف الموجود في momentum_chase — رأينا
                                              # فعلياً صفقات بزخم +33,742% و+54,008% تنهار بالكامل
                                              # (-100%) خلال 3-8 دقائق فقط. زخم بهذا الحجم يعني
                                              # غالباً "قمة انفجار مصطنعة" على وشك الانهيار، حتى
                                              # لو تكرر عبر قراءتين متتاليتين (كلتاهما قد تلتقطان
                                              # نفس القمة المصطنعة قبل الدمبينج مباشرة).
...
@dataclass
class EstablishedLiquidSettings:
    """
    استراتيجية مختلفة جذرياً عن كل الاستراتيجيات الأربع السابقة — بدل
    مطاردة عملات وليدة الدقائق الأولى، تستهدف عملات **أثبتت استقرارها
    فعلياً**: تخرّجت من Pump.fun منذ أيام، ولا تزال بسيولة عالية وتداول
    حقيقي مستمر. فلسفة مختلفة تماماً: "استقرار مُثبَت"، وليس "زخم لحظي".
    مبنية على طلب صريح بعد تجربة حقيقية: سرعة انهيار العملات الجديدة
    تتجاوز قدرة أي نظام مراقبة على الاستجابة، مهما كانت سرعته.
    """
    enabled: bool = True
    min_age_days: float = 5.0            # حد أدنى: 5 أيام منذ التخرّج قبل حتى التفكير بالدخول
    min_liquidity_usd: float = 50000.0   # أعلى بكثير من عتبة القنص السريع ($5,000) — استقرار حقيقي
    min_volume_h24_usd: float = 20000.0  # حجم تداول يومي حقيقي ومستمر، وليس نبضة لحظية
    max_drawdown_h24_pct: float = -40.0  # رفض إن كانت العملة **حالياً** في انهيار حاد (24 ساعة)
    trailing_stop_pct: float = 20.0      # وقف متحرك أوسع بكثير — هذه الاستراتيجية لا تواجه نفس
                                          # خطر "تبخر السيولة الفوري" الذي تواجهه عملات وليدة


ESTABLISHED_LIQUID = EstablishedLiquidSettings()
